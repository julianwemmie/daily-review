...
Daily Review uses [better-auth](https://www.better-auth.com/) for all authentication. There are three ways to authenticate: web sessions, API keys, and device authorization (for CLI login).
...
- **Stored by the CLI** in `~/.daily-review/config.json` (field: `apiKey`).
- **Sent as a header**: `x-api-key: <key>` on every API request.
- Can also be set via the `DAILY_REVIEW_API_KEY` environment variable.
...
The default `daily-review login` command uses the OAuth 2.0 Device Authorization Grant so the CLI can obtain a session token through the browser:
...
6. On approval, the server returns an `access_token`. The CLI saves it to `~/.daily-review/config.json` (field: `sessionToken`).
...
2. `DAILY_REVIEW_API_KEY` environment variable
3. Saved session token (from device flow) in `~/.daily-review/config.json`
4. Saved API key in `~/.daily-review/config.json`
