...
[{"front":"...","context":"...","tags":["..."]}]
CARDS_EOF
amber upload "$TMPFILE"
rm "$TMPFILE"
```

The CLI handles authentication automatically (env var `AMBER_CARDS_API_KEY`, config file, or session token from `amber login`).

If `amber` is not found, tell the user to install it:
...
1. Run `amber login` to authenticate via the browser, or
