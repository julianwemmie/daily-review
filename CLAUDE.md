...
3. **Review**: A web UI presents due cards. The user answers in free-form text. An LLM judge scores the response instead of comparing against a rigid "correct answer".
4. **Scheduling**: FSRS (Free Spaced Repetition Scheduler) manages when cards come back, based on the LLM judge's score.

## Architecture

- **Local** (Claude Code): Conversation parsing, card generation skill (`/flashcards` or `/daily-review`)
- **Cloud**: API + Postgres backend, review UI, LLM judge
...
- `front` (the prompt) + hidden `context` (reference material for the LLM judge, never shown to user)
...
The judge receives `front` + `context` + the user's free-form answer and produces a score (0-1), which maps to FSRS ratings:
- 0.0 - 0.3 → Again
- 0.3 - 0.6 → Hard
- 0.6 - 0.85 → Good
- 0.85 - 1.0 → Easy
